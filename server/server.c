...
int handle_private_message(server_t *server, int client_index, struct private_message *msg) {
    ...
    // Create private message to forward to target
    ...
    forward_msg.session_token = 0; // Not needed for incoming messages
    
    // Set sender as the "target" for the recipient (so they know who sent it)
    ...
    if (sent == -1) {
        printf("Failed to send private message to target client %d\n", target_index);
        send_error_response(sender->socket_fd, "Failed to deliver message");
        return -1;
    }
    
    printf("Private message delivered from %s to %s\n", sender->username, target_username);
    return 0;
}
